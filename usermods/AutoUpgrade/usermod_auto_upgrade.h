...
class AutoUpgradeUsermod : public Usermod {
  ...
  public:
    ...
    AutoUpgradeUsermod(const char *name, bool enabled):Usermod(name, enabled) {} //WLEDMM
    ...
    void setup() {
      if(enabled) {
        Serial.println("AutoUpgrade: enabled");
        // OsOta.handle();
      }
      else {
        Serial.println("AutoUpgrade: disabled");
      }
      initDone = true;
    }
